    ...
    switch (MIDI.getType()) {
      ...
      case midi::NoteOn:  //if NoteOn
        ...
        if (note_on_count == 2) {
          ...
          digitalWrite(5, HIGH);  //Gate》HIGH
          // OUT_MOD(cv[note_no]);   //V/OCT LSB for DAC》参照
        }
        break;
      ...
      case midi::ControlChange:
        if (MIDI.getData1() == 1)
          OUT_MOD(MIDI.getData2() << 5);  //0-4095 除(2^5=32) = 0-127
        if (MIDI.getData1() == 21)
          OUT_CV(MIDI.getData2() << 5);  //V/OCT LSB for DAC》参照
        break;
